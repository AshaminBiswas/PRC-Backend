...
export const notifyAdmins = async (
  ...
  try {
    ...
    // 2. Create in-app notifications in bulk for the Admin Console
    ...
    // 3. Dispatch Emails ONLY if explicitly requested via options.sendEmail === true
    if (options?.sendEmail) {
      ...
      logger.info(`[Admin Notification] Alert "${title}" dispatched via email to ${admins.length} admins.`);
    } else {
      logger.info(`[Admin Notification] In-app notification "${title}" created for ${admins.length} admins.`);
    }
