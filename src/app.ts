...
// ─── CORS (Must be FIRST middleware before compression/helmet) ───────────────
const corsOptions: cors.CorsOptions = {
  origin: (origin, callback) => {
    // Allow requests with no origin (mobile apps, curl, server-to-server, Render probes)
    ...
    // Allow localhost with any port (e.g. 5173, 5174, 5175, 3000, 3001) in all environments
    if (/^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(normalizedOrigin)) {
      return callback(null, true);
    }

    // Allow local Wi-Fi / LAN IPs (e.g. 192.168.x.x, 10.x.x.x, 172.16-31.x.x) for testing
    ...
    // Permissive fallback for authorized API clients
    return callback(null, true);
  },
  credentials: true,
...
// Explicit header injector to guarantee CORS headers on all downstream responses & errors
...
app.use(`${prefix}/b2b-orders`, b2bOrdersRoutes);
