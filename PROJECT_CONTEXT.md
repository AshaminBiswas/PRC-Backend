...
- **Mobile Native App & PWA Architecture**:
  ...
  - Dual-mode responsive view architecture across all core operational hubs (`sm:hidden` touch cards + `hidden sm:block` full tables):
    ...
    - **B2B & Commercial Operational Suite (`AdminSidebar.tsx`, `AdminLayout.tsx`)**: Unified enterprise workspace grouping 8 specialized commercial hubs:
      ...
      - **B2B Payments & Receivables (`AdvancePaymentsTrackerPage.tsx` under `advance-payments`)**: Redesigned, comprehensive, and responsive commercial command center across mobile, tablet, and desktop devices. Features:
        - **4-Stage Commercial Document Pipeline (`[QT] ──> [PO] ──> [PI] ──> [B2B Order]`)**: Full end-to-end commercial linkage visualizer rendered on both mobile touch cards and desktop table rows (`Commercial Pipeline` column) with interactive status badges and color-coded state indicators (`ACCEPTED`, `VERIFIED`, `CONVERTED`, `CONFIRMED`).
        - **1-Click Quick Inspector Modals**: On-demand slide-over/popover inspectors for Quotations (`QuoteInspectorModal`), Purchase Orders (`PoInspectorModal`), Proforma Invoices, and B2B Wholesale Orders (`B2bOrderInspectorModal`) showing complete line items, tax breakdowns, fulfillment branches, terms, and attachments without losing ledger context.
        - **Official B2B Wholesale Order Integration**: Direct linkage with `b2b_orders` API (`b2bOrdersApi.listB2BOrders`) replacing generic retail orders, displaying branch allocations, order fulfillment status, and balance dues.
        - **Bidirectional Payment Synchronization**: Real-time cross-document reconciliation where recording advance/balance payments on a PI automatically updates linked B2B wholesale order balances and status (`PAID`/`PARTIAL`), and recording payments on a B2B order (`POST /api/v1/b2b-orders/:id/record-payment`) auto-updates the linked PI.
        - **Multi-Document Universal Search**: Global search filter instantly matches Quotation # (`QT-...`), Customer PO # / Submission ID, Proforma Invoice # (`PI-...`), and B2B Wholesale Order # (`B2B-...`), in addition to company names, emails, and transaction UTRs.
        - **Executive Analytics & Actions**: 4 executive KPI cards (Total Outstanding Dues, Overdue Receivables, Collections Received, Active B2B Accounts), clean 2-tab view (`Invoices Ledger` and `Company Balances`), aging SLA countdowns, 1-tap WhatsApp payment reminder notice with bank remittance details, and unified Record Payment modal.
    ...
    - **UP — Private Factory Operations, Daily Expense & Inventory Module (`UPPage.tsx` under `up`)**: Private, factory-specific operations and financial ledger for the manufacturing facility, completely isolated from general corporate operations and displayed throughout the UI strictly as **UP**. Features:
      ...
      - **Executive Financial Dashboard & Reports**: Real-time KPI deck (Current Month Total, Today Spend, Verified Total, Unverified Total, Avg Daily Spend, Peak Expense), category breakdown with visual progress bars, 30-day expense velocity chart, and multi-filter Excel report generator (`UP_Expense_Report_YYYY-MM.xlsx`).
    ...
    - **Multi-Stock Management & Facility Hub (`InventoryPage.tsx`, `AddSkuPage.tsx`)**: High-performance multi-branch inventory control console featuring **0ms Stale-While-Revalidate (SWR) Instant Boot** (instant render from local storage cached snapshots for branches, suppliers, categories, and inventory metrics with background revalidation; zero blank screens or blocking spinners), **30-Second In-Memory Tab Cache** (`tabCacheRef.current` allowing instant tab switching across Stock Matrix, Purchases, Transfers, Movements, Facilities, Suppliers, and Reports), **7-Tab Enterprise Navigation Bar** (`stock`, `purchases`, `transfers`, `movements`, `suppliers`, `branches`, `reports`), **Dedicated Full-Page Add SKU & Stock Allocation Suite (`AddSkuPage.tsx` under `inventory-add-sku`)** (replaces popups with a dedicated operational page featuring mandatory SKU Name and Barcode, dynamic Finish selection `{SS, NA, NYLON}`, dependent surface Colour swatches `(Golden, Black, SS)` for SS, `(Black, NA Aluminium)` for NA, and `(Black)` for Nylon, optional size and dimension inputs for `height`, `width`, and `length` in millimeters, non-mandatory unit purchase cost and selling prices, destination warehouse allocation, live dossier card preview, and atomic catalog broadcast synchronization), **Enhanced Stock Update Suite (`StockEditModal.tsx`)** (quick delta adjustment chips `+1, +5, +10, +25, +50, +100, -1, -5, -10, -25`, live calculation preview bar showing on-hand units, reserved balances, available quantities, dynamic stock health badge, facility attribution, and custom audit note inputs logged directly to `StockMovement`), **Production Report Generator & Audit Center (`activeTab === 'reports'`)** (interactive time horizon control dock supporting Day-Wise, Week-Wise with Monday–Sunday span calculation, Month-Wise, Year-Wise, and Custom Range-Wise filters with Facility and Sales Channel scope selectors), **4 Auditor-Grade Production Report Export Suites** (Stock Matrix & Valuation Report, Stock Movements Audit Ledger with staff actor attribution, Itemized Purchases & Procurement Report answering who purchased, on which date, how much quantity, and Customer Orders & Stock Consumption Report answering who placed the order, on which date, how much quantity across B2C & B2B channels), **Dual-Mode Responsive View Architecture** (touch-friendly mobile cards dock with SKU badges, stock health meters, stepper controls, and quick action bar, paired with high-density desktop matrix featuring live health indicators, reorder levels, and facility badges), **Modular Extracted Modal Architecture** (`PurchaseModal.tsx`, `TransferModal.tsx`, `AdjustmentModal.tsx`, `BranchModal.tsx`, `QuickStockModal.tsx`, `StockEditModal.tsx`), and **Unified Stock-to-Catalog Cascade Deletion Protocol** (deleting a stock SKU executes a backend atomic write-off creating `StockMovement` audit trails, deletes all branch allocation rows, soft-deletes the product from the catalog with `deletedAt: new Date()`, `status: 'INACTIVE'`, `isVisible: false`, and `stock: 0`, zeroes out variants, purges client-side product caches, and broadcasts real-time deletion events via `BroadcastChannel("prc_products_channel")` to auto-delete the SKU from the product listing page and storefront).
