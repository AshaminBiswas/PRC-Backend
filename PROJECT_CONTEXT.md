...
- **Multi-Branch Live Stock, Concurrency-Guarded Sales & Catalog Search Engine (`inventory.service.ts`, `checkout.service.ts`, `orders.service.ts`, `ProductPicker.tsx`, `ProductsPage.tsx`, `InventoryPage.tsx`)**:
  ...
  - **Storefront (`D:\frontend`)**:
    ...
    - `CustomerProformaViewPage.tsx`: Public/authenticated verification and acceptance page (`/proforma/:token` & `/pi/:token`) allowing clients to accept terms, submit bank advance transaction reference (UTR / IMPS / RTGS), upload payment screenshot or bank receipt PDF (PNG, JPG, WEBP, PDF up to 10MB with live preview and removal), submit site remarks, and download official monochrome PDFs.
