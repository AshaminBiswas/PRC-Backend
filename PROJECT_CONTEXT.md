...
- **Mobile Native App & PWA Architecture**:
  ...
  - Dual-mode responsive view architecture across all core operational hubs (`sm:hidden` touch cards + `hidden sm:block` full tables):
    ...
    - **B2B & Commercial Operational Suite (`AdminSidebar.tsx`, `AdminLayout.tsx`)**: Unified enterprise workspace grouping 8 specialized commercial hubs:
      ...
      - **Payment Follow-up & Dues Recovery Cockpit (`AdvancePaymentsTrackerPage.tsx` under `advance-payments`)**: Enterprise receivables recovery cockpit and commercial ledger across mobile, tablet, and desktop devices:
        - **5-Tab Operational Cockpit**:
          ...
          2. `DOCUMENTS_LEDGER` ("Advance Payments & PI Receivables"): Original 4-stage commercial pipeline (`[QT] ──> [PO] ──> [PI] ──> [B2B Order]`), quick inspector modals for Quotations, POs, PIs, and B2B Orders, and bidirectional payment reconciliation.
          3. `CUSTOMER_ACCOUNTS` ("Company Ledgers"): Customer accounts summary, outstanding balances, and Statement of Account dispatches.
          4. `DISPUTED_ACCOUNTS` ("Declined & Disputed"): Dedicated isolation queue for accounts with rate disputes or collection suspensions, requiring mandatory justifications and protecting accounts from automated communications with 1-click "Resume Recovery".
        ...
        - **Legacy Customer Onboarding (`AddOldCustomerModal.tsx`)**: Onboards pre-existing clients (`users.source = 'OLD_CUSTOMER'`) with legitimate opening balances (`opening_balance_entries`) without creating fake orders or synthetic invoices. Features pre-flight duplicate checking against name, phone, email, and GSTIN.
        ...
        - **Resend Decoupled Email Engine & B&W Statement of Account PDF**: Dispatches official Statements of Account via Resend (`PAYMENT_FOLLOWUP_RESEND_*`) attaching pure black-and-white print-ready PDFs generated with `pdfmake` matching the PI/PO vector standard.
        ...
        - **2-Stage Bulk Actions (`BulkCommunicationModal.tsx`)**: Pre-execution preview breakdown detailing eligible accounts vs skipped accounts (missing phone, missing email, disputed/declined, already contacted within quiet period) followed by resilient sequential batch dispatch with summary results.
        - **360° Customer Dues Detail Drawer (`CustomerDuesDetailDrawer.tsx`)**: Slide-out drawer with open receivables table, payment allocations history, follow-up touchpoint timeline with PTP commitments, dispatched email logs, and SMS logs. Also embedded into `CustomerDossierPage.tsx` under dedicated `Dues & Recovery` tab.
