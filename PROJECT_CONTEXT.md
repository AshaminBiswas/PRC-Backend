...
| `barcode` | `/api/v1/barcode` | Production Code-128 Barcode & QR code streaming (`/image`, `/qr`), streamlined 58mm & 80mm thermal sticker PDF generator (`/label`, showing Company, prominent SKU, QR, and enlarged barcode without product details/MRP), camera/laser scan lookup (`/scan/:code`) with multi-branch stock levels, and order dispatch verification with physical inventory deduction (`POST /dispatch`). |
