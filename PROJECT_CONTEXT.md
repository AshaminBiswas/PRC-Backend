...
- **Mobile Native App & PWA Architecture**:
  ...
  - Dual-mode responsive view architecture across all core operational hubs (`sm:hidden` touch cards + `hidden sm:block` full tables):
    ...
    - **Employee & Payroll Management Workspace (`EmployeeManagementPage.tsx`)**: 5-tab enterprise HR hub (Employee Master Directory, Daily Attendance Matrix with 0ms optimistic updates, single-click status logging, Sunday shift approval, fast overtime stepper, dedicated attendance edit modal for detailed remarks/adjustments, Leave Ledger with automated monthly CL/EL accrual, Advances & Deductions tracking with advance taken date logging and full edit/delete modals, and Monthly Payroll Engine with formula calculation, Super Admin disbursement authorization, PDF payslip generation, and automated payslip emailing).
    - **Daily Cash Expense Tracker Workspace (`ExpensesPage.tsx`)**: 5-tab responsive operational cash management hub (Overview Dashboard with real-time cash balance card and daily/monthly spend analytics, Complete Expense Ledger with multi-filter drawer and inline approval/void workflows, Fast-Log Expense form with quick preset buttons, auto-approval threshold notifications, and receipt attachment, Float Top-Up & End-of-Day Physical Cash Reconciliation with interactive currency denomination counter ($\times 500, 200, 100, 50, 20, 10, 5, 2, 1$) and automated variance flags, and Multi-Sheet Server-Side Excel Generator for Day/Week/Month/Year expense reports with category breakdowns and professional styling).
